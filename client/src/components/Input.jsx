...
//  const {mddata ,setMddata} =useContext(inputContext);
const [mddata ,setMddata] = useState();
const {view, setView} = useContext(InputContext)
 
    useEffect(()=>{
  
        const fetchmd = async () => {
        try {
            // const result = await axios.post(`${url}/markdown`
            // console.log(mddata);
            if(mddata !== null && mddata !== undefined){
            const result = await axios.post("http://localhost:8084/markdown"
            ,{mddata});
            const newdata= result.data;
            setView({...view,newdata});}
...
<div>  
<form>
   <div className="m-4 w-[400px] h-[600px] min-w-0  sm:min-w-[400px] sm:max-w-[400px]  md:min-w-[600px] md:max-w-[700px] xl:min-w-[1000px] xl:max-w-[1000px] border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
